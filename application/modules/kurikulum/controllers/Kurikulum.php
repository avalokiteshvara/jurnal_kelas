...
class Kurikulum extends MX_Controller
{
    ...
    public function rekap_jurnal_kelas()
    {
        ...
        if (!empty($_POST)) {
            ...
            $jurnal = $this->db->query("SELECT a.id,a.tanggal,b.hari, b.jam, 
                                               c.nama AS pelajaran, 
            ...
            $rekap = $this->db->query(" SELECT c.nama_kelas, 
                                                SUM(IF(a.`status` = 'PENDING',1,0)) AS pending,
                                                SUM(IF(a.`status` = 'HADIR',1,0)) AS hadir,
                                                SUM(IF(a.`status` = 'DIGANTIKAN',1,0)) AS digantikan,
                                                SUM(IF(a.`status` = 'KOSONG',1,0)) AS kosong      
                                        FROM jurnal a
                                        LEFT JOIN jadwal_mengajar b ON a.jadwal_mengajar_id = b.id 
    ...
    public function edit_jurnal_kelas(){
        try {
            $this->load->library(array('grocery_CRUD'));
            $crud = new Grocery_CRUD();

            $crud->set_table('jurnal');
            $crud->set_subject('Edit Jurnal');

            $crud->field_type('tanggal', 'hidden');
            $crud->field_type('jadwal_mengajar_id', 'hidden');
            $crud->field_type('validasi', 'hidden');

            $extra = array('page_title' => 'Edit Jurnal');

            $crud->set_lang_string(
                'insert_success_message',
                 'Your data has been successfully stored into the database.<br/>Please wait while you are redirecting to the list page.
                 <script type="text/javascript">
                  window.location = "'.site_url(strtolower(__CLASS__).'/rekap-jurnal-kelas') . '";
                 </script>
            ...
            $this->breadcrumbs->push('Dashboard', $this->base_breadcrumbs);
            $this->breadcrumbs->push('Rekapitulasi Jurnal Kelas', $this->base_breadcrumbs . '/rekap-jurnal-kelas');
            $this->breadcrumbs->push('edit',$this->base_breadcrumbs . '/edit_jurnal_kelas/Edit/');
